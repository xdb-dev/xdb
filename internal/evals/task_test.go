  ...
  - name: month-end
    ...
    checks:
      ...
      - name: twelve transactions
        run: test "$(xdb records list xdb://household/transactions -o ndjson | wc -l)" -eq 12
rubric: |
...
func TestLoadTask(t *testing.T) {
	...
	assert.Contains(t, checks[2].Run, "wc -l")
}
...
func TestLoadTaskInvalid(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			yaml: "name: t\nnamespace: ns\nphases: [{name: p, prompt: x, checks: [{name: c, run: xdb, answer: '1'}]}]\n",
		...
		{
			name: "unknown key",
			yaml: "name: t\nnamespace: ns\nphases: [{name: p, prompt: x, checks: [{name: c, run: xdb, ndjson_cnt: 1}]}]\n",
			want: "ndjson_cnt",
		},
	}
