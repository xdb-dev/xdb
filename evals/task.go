...
// is set. A Run check runs a command in the sandbox and applies Expect.
// An Answer check compares the ANSWER line of the subject's final reply.
type Check struct {
	Name        string  `yaml:"name"`
	Run         string  `yaml:"run"`
...
// Expect is the assertion vocabulary for a Run check. Every set field must
// hold.
type Expect struct {
	Exit           *int           `yaml:"exit"`
	ExitNonzero    bool           `yaml:"exit_nonzero"`
	StdoutContains string         `yaml:"stdout_contains"`
	StderrContains string         `yaml:"stderr_contains"`
	StdoutEmpty    bool           `yaml:"stdout_empty"`
	JSON           map[string]any `yaml:"json"`
	NDJSONCount    *int           `yaml:"ndjson_count"`
	NDJSONIDs      []string       `yaml:"ndjson_ids"`
	Error          map[string]any `yaml:"error"`
}

// IsEmpty reports whether no assertion is set.
...
func (c *Check) validate() error {
	...
	if c.Run != "" && c.Expect.IsEmpty() {
		return errors.New("expect needs at least one assertion")
	}
