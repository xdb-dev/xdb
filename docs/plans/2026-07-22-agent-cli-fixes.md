...
The 2026-07-22 review (`docs/research/2026-07-22-agent-cli-review.md`) found mismatches between the CLI reference and its behavior. Dry runs wrote data, unknown commands exited successfully, and conflicting creates discarded new payloads. Batch and watch were advertised stubs, the bundled skill failed as written, and error codes varied across failure paths. The e2e scenario work (`docs/plans/2026-07-22-e2e-cli-scenarios.md`) also found JSON and array decoding failures, integer precision loss above 2^53, and inaccessible schema revision and `items` fields.

This plan addresses the 31 findings across RPC/core, storage/filtering, encoding, API, and CLI code. It also consolidates the 13 scenarios in `tests/e2e/scenarios.yaml` and adds regression coverage with each fix. The design used three exploration agents and two design agents; implementation mechanisms were checked against the referenced source locations.

Planning found additional bugs:
...
| CLI exit mapping | Unchanged contract: domain errors (NOT_FOUND, ALREADY_EXISTS, SCHEMA_VIOLATION, CONFLICT, NOT_IMPLEMENTED) -> 1; INVALID_ARGUMENT -> 3; CONNECTION_REFUSED -> 2; INTERNAL -> 4. Exit 3 becomes reachable |
...
- W3: wrap ALL describe RPC calls with `wrapRPCError("introspect", <action>, ...)`: listMethods/listTypes/describeMethod/describeType/describeDataSchema (+ParseURI via
...
- S8: `schemaFieldPayload` gains recursive `Items`; `schemaDefPayload` gains `Revision`
  ...
  revision). CAS uses the existing `schema.NextRevision`: revision 0 -> unconditional
...
- `convertToType` takes full `core.Type`; new TIDJSON case (marshal -> JSONVal) and TIDArray
  ...
  passthrough: prevents silent data loss); float -> int only when lossless.
...
- Tests: decoder_test tables: json field object round-trip; ARRAY<INTEGER> from [1,2,3];
  ...
  errors. Run the full `make test` suite to check for regressions from UseNumber.
...
- S10: accept namespace-depth `records.list` (depth 1-2 from Phase 2; facade ns-scan
...
- W5: describe offline fallback to `api/catalog` on connection error (`"source":
  ...
  core.ValueTypeNames: keeps the reference in sync); `describeMethod` gains `cli` section (flags walked
...
- Tests: describe_test (offline fallback table, cli-flags section, schema-format,
  ...
  (executes skill payloads against live test daemon: checks that the skill matches the implementation).
...
Phases 1-13 shipped as 13 commits on main. Phase 14 (`_updated_at`) was deferred because it required SQLite column changes and migration of existing tables. The completed work included a reserved-attribute filter allowlist, `checkMutation` validation, a decoder metadata-stripping hook, and a note excluding metadata from create-equivalence comparisons.
