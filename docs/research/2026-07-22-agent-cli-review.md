...
The CLI's agent-first *architecture* is right: JSON by default, structured error
envelopes with hints, documented exit codes, `describe` introspection, embedded
skills, clean stdout/stderr separation. The gap is *trust*: several advertised
surfaces are no-ops or unimplemented, several failure paths return exit 0, and
the error-code contract drifts badly outside the happy path. An agent that takes
the CLI's self-description at face value will validate with a `--dry-run` that
actually writes, plan around a `batch` that doesn't exist, and misread its own
input mistakes as server faults.

## What works well (verified)

- **Top-level `--help` is a complete mental model in one call**: URI scheme with
  concrete examples, all resources/operations/aliases, exit-code table, pointers
  to `describe` and `skills`. The blind agent completed a 7-step task with almost
  no flag-guessing.
- **Error hints close the loop**: `delete requires --force to confirm`,
  `is the daemon running? try: xdb daemon start`, NOT_FOUND suggesting the parent
  list. Self-healing errors worked exactly as intended.
...
- **Crash durability and idempotency**: `kill -9` on the daemon lost zero
  ...
  upsert / import-upsert retries are genuinely idempotent; export→delete→import
...
Severity: **P0** = breaks agent trust in the tool's own claims; **P1** = breaks
agent branching/planning; **P2** = friction.
...
1. **`--dry-run` is a silent no-op; mutations persist.** `records
   ...
   but only ever read in `batch.go` and `schemaimport.go`. An agent "validating a
   plan" is executing it. (Verified by hand.)
2. **Unknown command exits 0 and dumps markdown to stdout.** `xdb frobnicate` (or
   any typo'd verb) prints the 4.5KB "# XDB CLI Context" doc on stdout, exit 0 —
   the root `Action` in `app.go` treats any unmatched arg as a context request.
   Poisons `| jq` pipelines and hides typos; the single stdout-hygiene violation
   found. (Verified by hand.)
3. **Conflicting duplicate `create` silently drops the new payload.** Second
   ...
   the error catalog but unreachable. The schema case is worst: an agent believes
   its migration applied; revision stays 1. (Verified by hand.)
...
5. **The bundled `getting-started` skill fails at step 1 as written.** It teaches
   ...
   (`cli/skills/getting-started/SKILL.md:18`). The single most-trusted artifact
   for a blind agent fails verbatim — and fails as `INTERNAL` exit 4. Skills
...
8. **Three error shapes exist.** (a) the standard envelope; (b) fallback
   ...
   first.
9. **Silent-success variants of the same trust problem**: `import --create-only`
...
17. **`import` is fail-fast partial with no progress accounting**: mid-file
    violation commits earlier lines, stops, and reports only the failing line —
    no "imported N of M", no machine-readable summary (the human summary is
...
18. **No full-tree dump**: cold-start inventory costs `1 + N + M` calls
    ...
    only; `namespaces get` returns just the name. A recursive list or
    namespace-level export would make amnesia recovery O(1).
...
4. Make help/describe honest about `batch`/`watch` (or ship them); distinct
